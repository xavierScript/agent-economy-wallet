...
<Note>
  **Network Requirement:** Because the AI Buyer needs to send real HTTP requests
  ...
  service like Cloudflared, or deploying the Merchant to a cloud provider
  like Render.
</Note>
...
<Steps>
  ...
  <Step title="Start the Server & Expose it">
    ...
    *If your Buyer agent is on a different network, expose your local port 3000 publicly:*
    ...
    ```
    *(Copy the resulting Cloudflared URL or your Local IP to use in the prompts below).*
  </Step>
</Steps>
...
_Make sure to replace `<MERCHANT_URL>` with your Cloudflared URL._
