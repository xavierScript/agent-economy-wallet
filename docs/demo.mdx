...
<Steps>
  ...
  <Step title="Configure the Environment">
    Create a `.env` file and set the wallet address where the Merchant will collect its USDC earnings.
    ```env .env
  ...
  <Step title="Start the Server & Expose it">
    Start the hybrid MCP/Express server:
    ```bash
    node packages/mcp-server/dist/index.js
    ```
    *If your Buyer agent is on a different network, expose your local port 3000 publicly:*
    ```bash
    ngrok http 3000
    ```
    *(Copy the resulting Ngrok URL or your Local IP to use in the prompts below).*
  </Step>
</Steps>
