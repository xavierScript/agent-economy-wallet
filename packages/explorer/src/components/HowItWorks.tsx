...
const STEPS = [
    ...
    log: (
      <>
        <span className="terminal-cmd">
          &gt; agent-mcp tool call discover_registry
        </span>
        {"\n"}
        <span className="terminal-info">
          [Info] Scanning Solana network for active AI merchants...
        </span>
        {"\n"}
        [Wait] Parsing SPL Memo program transactions...{"\n"}
        [Ok] Found 14 matching agents offering "data_analysis"{"\n"}
        {`=>`} Target Agent PublicKey:{" "}
        <span className="terminal-success">7xDp...3qLp</span>
      </>
    ),
  },
  {
    ...
    log: (
      <>
        <span className="terminal-cmd">
          &gt; curl https://agent.target.local/.well-known/agent.json
        </span>
        {"\n"}
  ...
  },
  {
    ...
    log: (
      <>
        <span className="terminal-cmd">
          &gt; agent-mcp tool call check_reputation pubkey=7xDp...3qLp
        </span>
        {"\n"}
        <span className="terminal-info">
          [RPC] Querying Solana Reputation Program...
        </span>
        {"\n"}
        [Wait] Fetching staking and rating accounts...{"\n"}
        [Ok] Reputation Score:{" "}
        <span className="terminal-success">4.92 / 5.00</span>
        {"\n"}
  ...
  },
  {
    ...
    log: (
      <>
        <span className="terminal-cmd">
          &gt; curl -X OPTIONS https://api.agent.target/analyze
        </span>
        {"\n"}
        <span className="terminal-info">
          [HTTP] Probing x402 endpoint pricing...
        </span>
        {"\n"}
        ...
        {"\n"}
        {`=>`} Requested Price:{" "}
        <span className="terminal-warning">0.5 USDC</span>
      </>
    ),
  },
  {
    ...
    log: (
      <>
        <span className="terminal-cmd">
          &gt; agent-mcp policy check --amount 0.5 --currency USDC
        </span>
        {"\n"}
        <span className="terminal-info">
          [Auth] Evaluating spending policy for current session...
        </span>
        {"\n"}
        [Policy] Rule: max_spend_per_tx {`<=`} 5.0 USDC{"\n"}
        [Policy] Rule: allowed_merchants = "ANY"{"\n"}
        {`=>`} Policy Engine: <span className="terminal-success">APPROVED</span>{" "}
        (0.5 {`<=`} 5.0)
      </>
    ),
  },
  {
    ...
    log: (
      <>
        <span className="terminal-cmd">
          &gt; agent-mcp tool call pay_x402_invoice amount=0.5
        </span>
        {"\n"}
        <span className="terminal-info">
          [Tx] Constructing SPL Token Transfer...
        </span>
        {"\n"}
        ...
        <span className="terminal-success">tx_8a9B...4dEf</span>
      </>
    ),
  },
  {
    ...
    log: (
      <>
        <span className="terminal-cmd">
          &gt; curl -H "Authorization: Bearer tx_8a9B...4dEf"
          https://api.agent.target/analyze
        </span>
        {"\n"}
        <span className="terminal-info">
          [HTTP] Sending request with payment proof...
        </span>
        {"\n"}
        [Ok] 200 OK - Payment verified on-chain.{"\n"}
        {`=>`}{" "}
        <span className="terminal-success">
          Data payload received successfully.
        </span>
        {"\n"}
...
export default function HowItWorks() {
  ...
  useEffect(() => {
    if (activeStepRef.current && window.innerWidth <= 768) {
      // scrollIntoView was causing the whole page to jump.
      // We only want to scroll the horizontal list container to show the active step.
      const container = activeStepRef.current.parentElement;
      if (container) {
        const itemLayout = activeStepRef.current.getBoundingClientRect();
        const containerLayout = container.getBoundingClientRect();

        // Check if item is out of view horizontally
        if (
          itemLayout.left < containerLayout.left ||
          itemLayout.right > containerLayout.right
        ) {
          container.scrollTo({
            left: activeStepRef.current.offsetLeft - container.offsetLeft - 16,
            behavior: "smooth",
          });
        }
      }
  ...
  return (
    <section className="how-it-works">
      <div className="how-it-works-header">
        ...
        <h2 className="how-title">Autonomous Agent-to-Agent Payments</h2>
