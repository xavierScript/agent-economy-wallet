 ...
 *
 * MCP tool — fetch real-time token prices from the Jupiter Price API v2.
 * Returns USD prices for one or more tokens by symbol or mint address.
 * No API key required. This gives AI agents the market data they need
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `Error reaching Jupiter Price API: ${err.message}`,
            },
          ],
          isError: true,
        };
      ...
      if (!response.ok) {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `Jupiter Price API error (${response.status}): ${body}`,
            },
          ],
          isError: true,
        };
